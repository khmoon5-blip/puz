...
//            sprRenderer.FirstOrDefault().sortingOrder = 2;
        ...
        public void GenColor(IColorGenerator colorGenerator)
        {
            GenMarmalade();
            if (colorableComponent != null) colorableComponent.RandomizeColor(colorGenerator);
        ...
        private void Update()
        {
            ...
            if(!falling && square && LevelManager.GetGameStatus() != GameState.RegenLevel && field == LevelManager.THIS.field && !destroying)
                CheckSquareBelow();
        }
...
//        transform.position = destPos;// square.transform.position;
            ...
            if (!needFall)
            {
                ...
                if(!square.GetAllNeghborsCross().Any(i => (i.Item && i.Item.falling && i.Item.color == color)))
                {
                    ...
                    if(square != null && square.type == SquareTypes.JellyBlock)
        ...
        public void DestroyItem(bool showScore = false, bool particles = true, Item explodedItem = null, bool explEffect = false)
        {
            ...
            if (!explodedItem && (!marmaladeTarget || (!marmaladeTarget?.activeSelf ?? false)) && (combinationID == explodedItem?.GetHashCode() || dontDestroyOnThisMove /* || NextType != ItemsTypes
        .NONE*/ ||/* falling 
        ||*/ needFall) || !Explodable)
            {
                StopDestroy();
